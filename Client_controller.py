...
system_total = [
    80,
    0,
]  # This is a user setting setting default to 80kwh second is current total
...
def print_output():  # Prints our output in a human readable form
    ...
    for generator, data in output_dict.items():
        print(
            "Generator ID: {: >10} Output: {: >5.2f}Kwh State: {: >10}  Error: {: >15}  Flow Rate: {: >5.2f}%".format(
                generator, data[0], data[1], data[2], data[3]
            )
        )

    print(
        "\nCurrent system total: {: >2.2f}Kwh Desired output: {: 2.2f}Kwh  Satisfaction: {: >2.2f} %".format(
            system_total[1], system_total[0], satisfaction
        )
    )  #
...
def on_connect(client, userdata, flags, rc):
    ...
    calc_satisfaction()
    client.subscribe(env.public_data.format('+'))


def settings_handle(client, userdata, message):
    print(f"Recieved new configurations: {env.black_box.decrypt(message.payload.decode())} ")


def publish_settings(target):
    client.publish(
        env.private_settings.format(target), env.black_box.encrypt(system_settings.SerializeToString())
    )
...
def calc_adjustment():
    ...
    calc_satisfaction()
    target = 0
    [low_flow, high_flow] = min(output_dict.values(), key=lambda sub: sub[3]), max(
        output_dict.values(), key=lambda sub: sub[3]
    )
    [target_low, target_high] = [
        key for key, val in output_dict.items() if val == low_flow
    ][0], [key for key, val in output_dict.items() if val == high_flow][0]

    if satisfaction <= 95:
        target = target_low
        system_settings.flow_rate = (
            output_dict[target][3] + 1 + 2 * (satisfaction / 100)
        )
    ...
    if (satisfaction < 95 or satisfaction > 105) and satisfaction != 0:
        ...
        if system_settings.flow_rate < 0:
            system_settings.flow_rate = (
                0  # so we don't have generators turn into motors lmao
            )
        publish_settings(target)
        set_comm_msg(
            "Generator: {} Adjusted from {: >2.2f}% to {: >2.2f}%".format(
                target, previous_rate, system_settings.flow_rate
            )
        )
...
def process_stats(client, userdata, message):
    ...
    temp_stats.ParseFromString(env.black_box.decrypt(message.payload))
    op_state_str = ["Not Ready", "Ready", "Stand By", "Running"][temp_stats.op_state]
    error_state_str = ["No Error", "Recoverable", "Fatal", "Maintenence"][
        temp_stats.error_state
    ]
    ...
    if temp_stats.op_state == proto.OP_RUNNING and temp_stats.power_generation == 0:
        temp_stats.power_generation = output_dict[slave_client][
            0
        ]  # this is to avoid zeros after updates :)

    output_dict[slave_client] = (
        temp_stats.power_generation,
        op_state_str,
        error_state_str,
        temp_stats.flow_rate,
    )
    calc_satisfaction()
    print_output()
...
client.message_callback_add(env.public_data.format('+'), process_stats)
# power_plant
...
while True:
    ...
    if system_total[0] <= 0:
        system_total[
            0
        ] = 0.1  # lets pretend we have a standby total current, lazy way to avoid 1/0 error
